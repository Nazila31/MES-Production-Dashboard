...
async function handleStartStage() {
    ...
    try {
        ...
        showSuccessModal("Stage Started", "Production stage has been started.");
    } catch (error) {
        alert(error.message);
    }
}
...
async function executeStageAction(action) {
    ...
    try {
        if (action === "finish") {
            ...
            showSuccessModal("Stage Completed", `${capitalize(stage)} has been completed.`);
        } else if (action === "pass") {
            await passQC(currentProject.id);
            showSuccessModal("QC Passed", "Project passed quality control.");
        }
    } catch (error) {
        alert(error.message);
    }
...
async function submitRejectQc() {
    ...
    try {
        const response = await rejectQC(currentProject.id, notes, returnStage);
        closeRejectModal();
        ...
        showSuccessModal(
            "QC Rejected",
            `Project returned to ${formatStatusLabel(returnStage)}. Silakan mulai ulang proses tersebut.`
        );
    } catch (error) {
        alert(error.message);
    }
