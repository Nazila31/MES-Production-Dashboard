...
function ensureNavbarControls() {
    ...
    navbarRight.querySelector(".notification-wrapper")?.remove();

    const base = getBasePath();
    navbarRight.insertAdjacentHTML("afterbegin", `
        <div class="notification-wrapper">
            <button class="notification-btn" id="notificationToggle" type="button" aria-label="Notifications">
...
async function loadNotificationsDropdown() {
    ...
    try {
        ...
        listEl.innerHTML = notifications.slice(0, 5).map((item) => {
            ...
            return `
                <div class="notification-item ${unreadClass}" data-notification-id="${item.id}" data-read="${item.read ? "1" : "0"}" data-href="${href}" role="button" tabindex="0">
                    <i class="bi ${icon}"></i>
                    <div>
        ...
        listEl.querySelectorAll(".notification-item").forEach((item) => {
            item.addEventListener("click", async () => {
                ...
                try {
                    ...
                    if (href) {
                        window.location.href = `${getBasePath()}${href.replace(/^\//, "")}`;
                    }
                } catch (error) {
                    console.error(error);
                }
